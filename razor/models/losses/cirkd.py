...
class CriterionMiniBatchCrossImagePair(nn.Module):
    ...
    def forward(self, feat_S, feat_T):
        #missformer
        #feat_T=feat_T.reshape(2,128,128,64).permute(0,2,3,1)
...
class StudentSegContrast(nn.Module):
    ...
    def forward(self, s_feats, t_feats, labels=None, predict=None):
        ...
        labels = torch.nn.functional.interpolate(
            labels,
            (s_feats.shape[2], s_feats.shape[3]),
            mode='nearest')
        ...
        loss = self.loss_weight * self.pixel_weight * pixel_sim_dis + self.loss_weight * self.region_weight * region_sim_dis
        return loss
