...
def main(args):
    ...
    visualizer.set_dataset_meta(classes=PascalVOCDataset.METAINFO['classes'],
                            palette=PascalVOCDataset.METAINFO['palette'],
                            dataset_name='PascalVOCDataset')
    ...
    with recorder_manager:
        # test a single image
        result = inference_model_kd(model, args.img, args.img.replace('img_dir', 'ann_dir'))
    ...
    classes=PascalVOCDataset.METAINFO['classes']
    palette=PascalVOCDataset.METAINFO['palette']
    ...
    mmcv.imwrite(mmcv.rgb2bgr(pred),
                        f'./out_dir/vis_voc/{osp.splitext(osp.basename(args.config))[0]}/2007_000333_rdkd_lw1.jpg')
